...
### This program calculates Net Benefit in the validation cohort for each model derived on cohorts of sample size 1434 
...
for (k in 1:1000){
  ...
  ## Want to reduce development data to a random subset of 1434 patients, called random.dat
  ## Extract 1434 row numbers at random
  row.ids <- sample(1:nrow(data_devel),1434, replace = FALSE)
...
save.image("R_out_C4/NB_bootstrap_female_1434.RData")
print("saved")
